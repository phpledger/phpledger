...
DB::useDB($database);
$migrated=pl_migrate();
cash_party_probe_assert(in_array('057_document_parties',$migrated['applied'],true) && in_array('058_money_account_kind',$migrated['applied'],true) && in_array('059_bank_overdraft_limits',$migrated['applied'],true),'Expected all three additive migrations to apply.');
...
cash_party_probe_assert($policy['cash_shortfall_policy']==='warning','Upgrade silently enforced strict cash policy.');
pl_save_trading_policies($actor,$company,$book,array_replace($policy,['cash_shortfall_policy'=>'strict','reason'=>'Explicit administrator choice in upgrade probe','idempotency_key'=>'upgrade-strict-policy']));
