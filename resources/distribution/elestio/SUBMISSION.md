# Elestio submission dossier — PHP Ledger

Elestio publishes no open packaging schema or self-service submission form
(confirmed from elest.io and docs.elest.io — see `NOTES.md`). New applications
are added to their catalogue through outreach to the Elestio team, who build
and manage the resulting deployment themselves. This dossier is written to be
pasted into that outreach, with `compose.yaml` attached as a working reference
deployment.
...
base) plus MySQL 8.4; it is the same runtime already used for
`compose.production.yaml`'s own production deployment guidance in
`docs/CONTAINER.md`, which targets ordinary small-VPS-class shared hosting
rather than a high-throughput workload.
...
exposes the same check to Elestio's orchestration.
...
TODO (owner to fill in): the maintenance/support contact Elestio should list
for this listing.
