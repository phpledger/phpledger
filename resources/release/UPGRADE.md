...
### 1.3 membership role cleanup

Migration 056 retires the legacy pl_company_members.role ENUM. Existing role_id assignments and custom grants are preserved; missing references from old integrations are backfilled from their legacy system-role label before conversion. The old role column remains as a deprecated text snapshot so the published 1.2.1 updater can verify that original data was preserved; it does not grant permissions and is not updated by role changes. SQL integrations must use role_id, preferably through the ordinary role-assignment service. Files-only downgrade is unsupported: restore the matched application/database backup to return to 1.2. Historical migration files remain unchanged.
