...
A **preview** on the preview channel, not a stable release. It never moves `latest` or the stable feed entry, and it carries no media kit: only a major release does. Its migrations, `035` through `040`, cannot be reverted, so it is published for engineering verification on disposable copies only: to prove the work is installable, upgrade-proven and signed. It is not a build for user or accountant testing; that review is booked on the release that ships with a refreshed demo. The current stable release remains 1.1.3, and the pilot stays on it; the hosted demo still runs 1.1.0. The new accounting behaviour below has not been reviewed by an external accountant; it has been through an internal accounting review, whose five material findings are all fixed in this build and described below.
