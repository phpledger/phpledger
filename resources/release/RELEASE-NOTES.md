...
- **The 1.1.2 notice is lifted for 1.1.3 onward.** An installation on 1.1.0, 1.1.1 or 1.1.2 with the publisher key pinned installs 1.1.3 from `/maintenance.php` once its metadata is signed; from 1.1.0, or from a 1.1.1 installed by replacing files without running `install/migrate.php`, that run also applies migration 034. The manual procedure (replace the files, run `php www/phpledger/install/migrate.php` once) remains available and is unchanged. The pinned maintenance loader `public/maintenance.php` is byte-identical to 1.1.0, 1.1.1 and 1.1.2, which is what allows the in-app path.
