...
- **A plugin runtime.** Hooks and filters in the WordPress style that may change core behaviour, a loader that checks each plugin's digest on every boot and deactivates one that no longer matches, a safe mode, exact-version requirements, one shared options table alongside each plugin's own tables, and Admin › Packages. A hook is blocked while the book lock is held: a validation filter runs before the book row is taken and may veto, and the after-commit hooks are flushed outside every lock. A veto leaves no journal, no line and an unchanged trial balance, and a hook that fails after the commit leaves the entry posted. **Channel plugins register as consumers of the existing outbound queue; no connector ships, and none is planned: they are for third parties to build.**
