...
**1.4.6** corrects three things the owner rejected in 1.4.5 on review (26 September 2026): the Start stage's sample gallery lists all eleven sample companies from a bundled catalogue snapshot, with in-place installation for an installation administrator; a partnership gets a capital and drawings account for each partner from business setup onward, split by the partners' ratio; and the balance sheet, profit and loss and trial balance use the standard layout, with no repeated class row and no zero lines. See RELEASE-NOTES.md for full scope. There are no new migrations or schema changes, and this patch carries no media kit.

**1.4.5** rebuilds the installer, business setup, first Home, shell and Packages page from the owner's review of 25 to 26 September 2026: one-screen steps with plain-language help, legal forms in each country's own words with the country of registration recorded (migration 061), named bank and cash accounts with opening money, a strict cash policy for new businesses, a mandatory installation notice, a getting-started guide on Home, inlined icons, and Packages with tabs and per-business switches. The candidate passed 708 application tests with zero failures on MySQL and 708 on MariaDB before the exact-package gates.

**1.4.1** includes consistent date entry, employee date-of-birth validation, eligible invoice/bill line accounts, required-field presentation, accurate sample completion counts, and navigation and phone layout corrections. The exact archive passed fresh installation and populated 1.4.0 upgrades on MySQL and MariaDB, with 699 full-suite tests passing per engine. See RELEASE-NOTES.md for scope and assurance limits. There are no new migrations or schema changes, and this patch carries no media kit. Independent accounting/security review, real-business pilots and unfamiliar-operator observation remain separate assurance work. It is not a completed ERP or a country-certified accounting product.
