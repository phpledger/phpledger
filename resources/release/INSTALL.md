...
On a server that writes a binary log, which MySQL 8 does by default, the database account also needs permission to create triggers there. Set `log_bin_trust_function_creators = 1` under `[mysqld]` in `my.cnf` or `my.ini`, or in the parameter group of a managed database. Without it MySQL refuses `CREATE TRIGGER` for an account that lacks `SUPER`, and PHP Ledger's immutability guards cannot be installed. Setup checks this and names the parameter before it runs any migration, so nothing is written to a server that cannot finish.

Use a dedicated database account on customer hosting; never put MySQL root in the configuration of a site other people can reach. On your own computer the installer accepts what local stacks provide, including `root` with no password, and creates the database when it does not exist yet; neither applies to a database on another server. Installation requires permission to create/alter the package's tables, indexes, triggers and views and to write migration receipts. Have the database administrator provision these privileges, then restrict the normal runtime account after validating the required workflows. Browser setup does not ask for a second account: enter one account, and edit `user` and `password` in `www/phpledger/includes/config.local.php` afterwards if you restrict it. Restricting it turns off the automatic update path described below. This preview does not prescribe an independently verified minimal grant set for every customer host. The two effective-source views use SQL SECURITY DEFINER with the account that runs migration 016. Use a stable dedicated migration account, preserve its access to the underlying tables, and verify the normal runtime account can select the views. Do not delete the view-definer account after setup; review view and trigger definers when rehearsing a restore.
