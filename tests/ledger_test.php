...
test('company setup creates one book, an owner, an annual period and scoped template', function (): void {
    ...
    // number it carried before the conversion (B56), so the fixture map holds both keys. A book is
    // also born with the chart's nineteen class and group headings: names for the report tree,
    // which take no posting and carry no purpose.
    // Seven aggregate payroll purposes supplement the fourteen original posting accounts.
    assert_same(21 + 19, (int) DB::queryFirstField('SELECT COUNT(*) FROM pl_accounts WHERE company_id = %i', $fixture['company_id']));
    assert_same(21, (int) DB::queryFirstField("SELECT COUNT(*) FROM pl_accounts WHERE company_id = %i AND code NOT LIKE '_-___-00000-__'", $fixture['company_id']));
    assert_same(21 + 19 + 6, count($fixture['accounts']));
