...
try{
    ...
    $check(trim(file_get_contents($baseline.'/www/phpledger/VERSION'))==='1.3.0','baseline artifact identifies published 1.3.0');
    $check(trim(file_get_contents($candidate.'/www/phpledger/VERSION'))==='1.4.0','candidate artifact identifies 1.4.0');
    ...
    if($mode==='fresh'){
        ...
        $code='require '.var_export($candidate.'/www/phpledger/includes/bootstrap.php',true).'; require '.var_export($candidate.'/www/phpledger/install/migrate.php',true).'; $m=pl_migrate(); if(!in_array("060_cash_balance_policy",$m["applied"],true)){throw new RuntimeException("Final migration absent");} $u=pl_create_user("fresh@example.invalid","Sample fresh owner","Sample-fresh-password-123!"); $f=pl_create_company($u,"Sample exact archive","USD","2026-01-01"); $j=pl_post_journal($u,$f["company_id"],$f["book_id"],["date"=>"2026-09-23","currency"=>"USD","source_type"=>"sample","source_reference"=>"exact-fresh","description"=>"Exact archive proof","idempotency_key"=>"exact-fresh","lines"=>[["account_id"=>$f["accounts"]["1000"],"debit"=>"1.2500","credit"=>"0"],["account_id"=>$f["accounts"]["3000"],"debit"=>"0","credit"=>"1.2500"]]]);$t=pl_trial_balance($u,$f["company_id"],$f["book_id"]);if(!$t["balanced"]||$t["total_debit"]!=="1.2500"||pl_migrate()["applied"]!==[]){throw new RuntimeException("Fresh accounting/replay failure");}echo "PASS exact artifact fresh migrations, owner, company, balanced posting and replay\\n";';
        ...
        if($mode==='cli'){$run([PHP_BINARY,$repository.'/tools/verify-m17-upgrade.php','check',$candidate],$env);}
            ...
            $manifest=json_decode(file_get_contents($baseline.'/PACKAGE-MANIFEST.json'),true,64,JSON_THROW_ON_ERROR);$check($manifest['version']===($mode==='recovery'?'1.3.0':'1.4.0'),'terminal package manifest has expected release');
