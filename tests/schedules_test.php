...
test('scheduled releases reject altered vectors and generic corrections through the central funnel',function():void{
    ...
    $other=ledger_fixture();sample_membership_insert(['company_id'=>$f['company_id'],'user_id'=>$other['actor_id'],'role'=>'viewer']);assert_throws(fn()=>pl_scheduler_retry($other['actor_id'],$f['company_id'],$f['book_id'],(int)DB::queryFirstField('SELECT id FROM pl_scheduler_jobs WHERE book_id=%i',$f['book_id']),'Forged'),DomainException::class);
});
