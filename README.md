...
PHP Ledger is open-source, self-hosted double-entry accounting software with a simple cash point of sale for small businesses, built on PHP 8.2+ with MySQL 8.4 or MariaDB 10.4+. **1.1.1** is the current release; **1.0.0** was the first stable release.
...
| **1.1.1 (published) and a proposed 1.1.2** | 1.1.1 shipped the six-stage Workbench installer, local-database and plain-HTTP handling, the trigger preflight with resumable migrations, the single database account and the corrected demo labels. It also carries migration 034 and the optional Stock locations module, which its notes wrongly call "no migration"; the demo packs did not leave the ZIP (issue #71 moves to 1.2). A 1.1.2 patch is proposed to correct the record, fix issue #88 and restore signed in-app updates before the 1 October pilot. |
