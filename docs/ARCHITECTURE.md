...
**Still M11.** The roughly two thousand existing interface strings are not externalised; the sweep counts them rather than failing, so the count can be driven to zero. Also outstanding: the Urdu and Arabic catalogues themselves, resolving a user's or company's locale preference (nothing calls `pl_set_locale()` in a request today — `PL_LOCALE` is the only lever), the RTL stylesheet and navigation review, script/font coverage, South Asian digit grouping and localised month names through the seams above, and translated print templates. A language is supported only after its catalogue, screens, dates and numbers are reviewed; none of the groundwork above claims a supported language.
