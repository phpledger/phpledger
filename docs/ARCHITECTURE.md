...
A **role** is a row in `pl_roles`. `company_id` NULL means an installation-wide definition, available to every company: the three protected system roles (Owner, Accountant, Viewer) are exactly those, and are read-only in the service, in the model and in the database, where migration 040 installs guard triggers. A company may define its own roles from the same catalogue. Roles are assigned per company (owner decision B17), on `pl_company_members.role_id`. The 1.1 three-value ENUM on that table is kept mirrored through 1.2 — every membership write sets both, and `pl_role_enum_mirror()` maps a custom role to `accountant` or `viewer` by whether it can write — and is dropped in 1.3.
