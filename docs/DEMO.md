...
The shared configuration uses a distinct `<project>_shared` internal network (default `10.204.83.0/24`, proxy `10.204.83.10`) and new `shared_demo_*` volumes. Preserve the older demo network and volumes for rollback; never reuse its outward-facing default network for the isolated application. Check both subnets against the host before cutover. The same Compose project retains the established web/database/scheduler container names, while the edge proxy takes over the existing loopback port after the old web service is stopped.
