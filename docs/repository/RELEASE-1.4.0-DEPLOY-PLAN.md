...
## Deployment preflight corrections

The first staged attempt stopped before any runtime or routing change because Docker returns mount arrays in varying order. Three independent reads recovered every original container fingerprint by reordering mounts only; all configuration-file and Nginx hashes remained unchanged. The operator now sorts mounts by destination while hashing every field. The second attempt stopped at a too-narrow proxy-count assumption: the live vhost has the demo route and two OAuth discovery routes. The corrected operator requires all three exact location/upstream pairs, retains their path suffixes and changes only their ports plus the website root. Eighteen local operator tests passed, including actual mount-change rejection and unexpected-route rejection; the complete corrected in-memory preflight passed against the actual host before preparing immutable attempt r3. Both earlier staged trees remain retained and were never launched. The execution wrapper uses pipelined SFTP with mandatory close/error handling and complete remote byte-hash verification; its 15 tests pass. These are deployment-helper corrections, not application/package changes.
