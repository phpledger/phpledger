...
The installer rendered the notice checkbox selected by default even though `PL_INSTALL_NOTICE=0` suppresses the send. The walkthrough now names the visible checkbox and instructs users who want it off to uncheck it. The check used both boxes off, and no installation notice or named registration was sent.
