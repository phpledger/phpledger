...
- **Optional shared Inventory:** one product identity, one default location per company (further warehouses and vans through the optional Stock locations module), one base unit per product, exact moving weighted-average costing, receipts/issues/returns, reasoned counts and reviewed value adjustments. Inventory and general-ledger balances reconcile from immutable movements.
...
- **Separate plugins:** quotes are excluded from this build. Batches, expiry, serials, landed costs and letter-of-credit workflows are also excluded. Multiple stock locations and transfers, and the trading-document extensions, arrive in 1.2 as bundled modules over the shared Inventory and AR services (decision B21). Future verticals compose the shared services instead of duplicating receivables, payables or stock engines.

Finish and verify these thin end-to-end flows together before starting another infrastructure-only milestone. A service business can use AR/AP with Purchasing/Inventory off; a stock business can order, receive, bill, sell, collect, credit/return and reconcile through shared services. The existing POS showcase at `/pos` keeps its sample catalogue and issues no inventory and no COGS; from 1.2 the counter till at `/counter` is the one that sells a location's real stock, and it does so as an ordinary customer invoice rather than through a second checkout path.

Current limits remain explicit: a receipt or payment settles one party's open items in one currency and one direction, allocated explicitly across up to 100 open items (raised from 30 in 1.2; a larger recovery is split into several receipts). Since 1.2 a receipt may leave an unallocated remainder, which is recognised in the same journal as unapplied credit on the customer-advances control — an ordinary liability, not a credit balance inside receivables and not a suspense account (B59). Unapplied credit is applied to later documents with no bank line, refunded in cash, or created by a credit note that has no original invoice; supplier advances mirror all three (B39). A batch of receipts posts one voucher per party row, never a shared multi-customer voucher (B57). What is still absent: no separate sales dispatch/reservation workflow; exact one-bank-row/one-journal-line matching; functional-currency inventory valuation; no automatic write-back of aged unapplied credit to income, which stays a reviewed journal. Original-cost purchase returns can require a reviewed value adjustment after intervening stock activity. Opening conversion links existing balances without reposting them. Outgoing foreign-bank carrying-value allocation — including the cash refund of a foreign advance from a foreign bank — rate providers, revaluation/consolidation and statutory reporting remain deferred. The representation of advances is with the accountant for review under B30; see the [worked examples](accounting/ADVANCES-AND-REFUNDS.md).

Document numbering (1.2, decisions B37 and B55): each of the four document types has one number series per company and book, set in Admin › Numbering. Its limits are equally explicit. Drafts carry no number; the number is issued on posting and is never reused or released, so a document that is never posted consumes nothing. The prefix, padding, year segment and reset rule can change at any time, but the next number can only be raised. A series that resets yearly adopts the year of its first document and then only moves forward, so a document dated in an earlier year than the series has reached is refused rather than mislabelled; switching that series to "never" is the documented way back. There is no per-warehouse, per-van or per-user series, and stock documents join the same scheme in M4 rather than gaining one of their own. Documents posted before migration `035_document_series` keep their derived `INV-<id>` number for ever; numbering is not applied retrospectively.
...
|---|---|
| Required accounting core | The employee master is locally implemented for 1.3 M17: company-scoped identity, effective-dated status, optional static pay terms and explicit ownership-person link; payroll calculations remain separate (#98). Companies/books, identities and permissions, chart and mappings, exact money/dates, journals/posting/audit, periods, opening/cutover, cash/bank reconciliation and reports. AR and AP are separately implemented required modules; generic manually configured tax calculation is shared core. From 1.2, the company's legal form and registration profile and the ownership register (owners/members, directors/officers, share classes, share ledger) are core (decision B63, issue #92); statutory forms, certificates and filing calendars are country plugins. |
...
| Optional business modules (bundled) | Bundled Purchasing and shared Inventory; from 1.2, bundled stock locations with transfers and bundled trading-document extensions (packs, line discounts, number series, sales-staff and area dimensions); future advanced stock/import features, country adapters, shop POS and storefront. Each owns its operations while using shared AR/AP, tax, inventory and core posting services. |
...
Structured account codes, contra accounts and owner transactions are **required accounting core**, not an optional module: the numbering, the contra presentation and the Owner and partners screen stay available with every optional module disabled, and no plugin may introduce a second numbering scheme or its own owner ledger. Known limits of this core as built: the chart is a three-level hierarchy with one optional sub-account level and no deeper nesting; a contra account is a presentation flag on an ordinary account, not a separate account type; profit allocation between AOP partners is not posted by the application and remains an accounting decision (owner decision B30); interest on an owner's loan, closing drawings to capital at year end, and partner current accounts separate from fixed capital are not modelled.
...
| 11. Distribution operations (directory plugin) | Route and van orders, evening settlement, collection sheets and handoff reconciliation, built over the 1.2 bundled modules (stock locations, packs, dimensions, number series) and core on-account receipts. | Van stock, documents, cash/credit and ledger reconcile across handoffs and retry/failure cases. |
...
| `inventory-locations` (1.1.0 from 1.2 M4) | `core 1.0.0`, `inventory 1.0.0` | `inventory_location_test.php` (masters, transfers at carrying value, per-location balance/history/valuation/counts, returns pinned to the original warehouse, disabled-with-non-default-stock behaviour); `stock_document_test.php` (per-kind numbering, a gate pass moving and posting nothing, warehouse kinds, transfers at carrying value unchanged, the driver's day end to end, the settlement approval permission, the location reports reconciling to the movements and the ledger, disable and re-enable with van stock present, the print routes per role, and the API/MCP reads); `tools/verify-upgrade.php stable-1.1.1` for the upgrade proof. |
...
| `fixed-assets` (1.0.0 from 1.3 M14) | `core 1.0.0` | `asset_test.php`: the schedule exactness matrix (both methods, annual, quarterly, monthly and stub-first-year books, and lives of 3, 7, 11, 13, 36, 37, 41 and 100 months, each asserted to charge exactly its life in months and to sum to cost less residual value), the acquisition posted through `pl_post_journal()`, a two-period worked example with a mid-year purchase and a sale at a loss, the idempotent depreciation run, a closed period refusing a run, the linked reversal and the difference on re-run, disposal by sale and by scrapping, the contra accumulated-depreciation rule and its balance-sheet presentation, the append-only event triggers, the register naming a control account the ledger disagrees with, the whole-month convention charging a full month whatever day an asset arrived and nothing in the month it leaves, the cost account landing under whichever group the `core.group.asset.ppe` key names and the fallback when no chart carries it, disable and re-enable with assets present, the module-side checksum-mismatch branch, and a guard refusing any declared route whose first segment is a real directory under the document root. |
