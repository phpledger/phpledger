...
Setup runs in four named steps, Database, Build, Account and Done, after a start page that checks this server and shows each requirement as its own light; anything missing becomes a screen of its own naming the places it is actually fixed, and nothing is saved until it is cleared. Every field has a question mark that explains it in plain words, the build starts by itself once the database answers, a settings file your host will not let PHP write becomes its own step with instructions for your kind of hosting, and the last screen shows your sign-in details.
...
The core includes the neutral starter. Optional multi-year histories are separate CC0 data-only packages. Business setup asks five things: what you are starting from (a new business at zero, a sample company's structure with zero balances, past records through the opening cutover, or a full fictional sample in a separate practice company); the business, with its legal form in the words of its country of registration and the numbers its invoices print; its owners and its bank, till and petty-cash accounts, each a named account under Cash and cash equivalents, with optional opening money recorded as capital introduced or an owner loan; the features to switch on and the account names; and a review, so nothing exists until you confirm. Sample companies appear as cards with their story, and an installation administrator can install one from the phpledger.com directory without leaving setup; Packages still lets you refresh the directory and review a sample first. Nothing downloads on page load. Exact dependencies must be installed separately. Removing the source package keeps existing companies and journals. Shared demo packages are verified host preloads and read-only.

The first Home shows a getting-started guide read from your data: invoice details, owners, bank and cash accounts, money in, customers and suppliers, and the first sale or expense. Each step links to the screen that does it, and the guide goes away by itself when the last step is done. Expense and Bill wait until a bank or cash account holds money; receipts, invoices and journals work from the start.
