...
MySQL 8.4 or MariaDB 10.4+ with InnoDB, the required extensions and HTTPS remain requirements; plain `http://localhost` is accepted only on your own computer. The test battery passed on MySQL 8.4 and MariaDB 10.4, 10.6, 10.11 and 11.4; MySQL 8.0 is not supported yet. A PHP selector in a hosting panel does not by itself prove compatibility.
