...
Run financial suites serially within each database. Separate Compose projects, non-overlapping subnets and isolated databases allow MySQL and MariaDB verification concurrently. The Python package allowlist gate remains separate: `python tests/package-builder-test.py`. After changing demo packs, regenerate their dependent sample structures with `python tools/build-sample-structures.py --write`; both `python tools/build-demo-packs.py --check` and `python tools/build-sample-structures.py --check` must pass. The extracted-package probe also rejects missing or stale bundled structures, preserving the runtime's source-digest guard.
...
- **Van settlement (1.2 M4, extended in 1.2.1 M9):** `pl_van_day(actor, company, book, warehouseId, date)` builds the driver's day from the movements themselves — opening, loaded, sold, sellable customer returns, returned to the warehouse, closing — and flags each product where `opening + loaded + customer_returns − sold − returned` does not equal the closing stock. Since M9 it also carries the money: `sales` is the day's posted sale documents raised from that van, split into cash and credit, and `credit` is the limit check. `pl_review_van_settlement(..., warehouseId, date, reason, key)` records that sheet; `pl_approve_van_settlement(..., id, reason, key)` approves it and refuses a day that does not reconcile, a day that put a customer outside their credit limit, or a day that changed since it was reviewed. Neither posts anything: the cash and credit belong to the sale documents the van raised, which posted when they were raised. **Approval is the `settlement.approve` capability**, which the Owner role holds by default; `pl_van_settlement_can_approve()` is the same question without the throw, for a screen deciding whether to show the button.
- **The driver's money and credit limits (1.2.1 M9):** `pl_van_day_sales(actor, company, book, warehouseId, date)` reads the posted invoices and customer credits carrying that van as their warehouse on that date, and splits each into what was paid and what was left on account. `pl_van_credit_check(company, book, sales)` then asks, for every customer who took credit that day, what they owe in total as of the settlement date (`pl_party_receivable_exposure()`) against the limit recorded on their financial profile (`pl_party_credit_limit()`). A **NULL** limit means none is recorded and is not a breach; a recorded **0.0000** is a real limit and any credit breaks it. `pl_party_credit_headroom()` is the forward-looking form the counter till uses before a sale. The check is at settlement rather than on the road because a van may be offline — research decision 4.
...
- **Stock reports (1.2 M4, B35):** `pl_stock_by_location(actor, company, book, asOf?, locations = 'all'|'fixed'|'mobile', search?)` groups warehouses then vans, each with its own subtotal, an aggregate quantity per item and a grand total reconciled against `pl_inventory_valuation()`. `pl_van_stock_report()` is the same report filtered to `mobile`. `pl_stock_location_aggregate()` is the across-locations counterpart, which is the only view that ties to the inventory control accounts. Cost columns follow `pl_stock_cost_visible()`, which since M7 is the `cost.view` capability and the per-report Admin setting together (owner decision B58), and are returned as `null`, not merely hidden in the template.
...
Focused validation: `docker compose --profile test run --rm test php tests/run.php --suite=foundations`. The complete `composer check` includes these suites. Upgrade verification accepts `fresh`, `foundation`, `core-0.1.2`, `opening-local`, `preview-0.2.1`, `preview-0.5.0`, `stable-1.1.1` and `converted-chart`. The `converted-chart` baseline starts at `035_document_series` with a chart that still uses its own account numbers, then proves the 036 conversion and the `040_contra_accounts_and_partner_identity` follow-up: old numbers kept, `is_contra` set only where a `semantic_key` determines it, nothing deduced from an account's name, the confirm step created and holding the owner screens shut, the reviewed answer marking exactly the accounts it named, and owner transactions working afterwards with a balanced trial balance and a no-op replay. **Every baseline passes.** The six that were broken were fixed on 21 September 2026; they were fixture problems, not upgrade problems, and nothing about the migrations changed. A baseline seeds the era it simulates instead of calling a helper that is newer than that schema: the actor is inserted directly, because `pl_create_user()` writes `pl_users.username`, which `032_user_names` adds; `preview-0.5.0` and `stable-1.1.1` seed their company and the bundled 1.0.0 chart directly, because `pl_create_company()` writes `legacy_code` and `is_contra`, which `036` adds, and because today's bundled chart is already numbered in the shape 036 converts to. `stable-1.1.1` seeds its whole stock state (both warehouses, the product, the receipt movement and the transfer) the same way: the `inventory-locations` manifest is now 1.1.0 and declares `038_stock_documents`, and every warehouse read names the columns 038 added, so today's stock services correctly refuse a 1.1.1 schema. **When you add a migration or change a bundled resource, expect to touch these fixtures**: a migration that provisions rows in an existing book, renumbers a chart or grows a module manifest changes what a prior-release database looks like afterwards, and the baselines state that outcome exactly rather than asserting that nothing moved. `foundation`, `core-0.1.2`, `opening-local` and `preview-0.2.1` still seed today's bundled chart, because the review step they exercise maps every starter purpose onto a separate existing account; their charts are therefore already numbered and 036 has nothing to convert in them. `converted-chart`, `preview-0.5.0` and `stable-1.1.1` are the baselines that prove the conversion. Run `tools/verify-currency-upgrade.php` with the same disposable-test root invocation as the existing upgrade verifier to exercise interrupted backfill, second-connection guards, recovery and legacy request replay. Only randomly created databases in `db_test` are touched by those upgrade verifiers.
...
### Counter point of sale and the distribution simulation (1.2.1 M9)

**The till is not a parallel path.** `pl_post_counter_sale()` posts through `pl_save_and_post_ar_document()`, so a
counter or van sale is an ordinary customer invoice: the stock leg is the invoice's own issue service off the selected
warehouse's balance, the money is the existing recognition journal, and a cash sale's `cash_received` is settled by
`pl_settle_open_item()` inside the same posting action. There is no counter-sale table, no counter-sale journal and no
counter-sale number series. Do not add one: add a field to the invoice instead.

| Signature | What it is for |
|---|---|
| `pl_counter_pos_catalogue(actor, company, book, warehouseId, search?, asOf?)` | what that location is actually holding, with its packs; carrying value withheld without `cost.view` |
| `pl_counter_sale_input(input)` | pure normaliser: tender, location, customer, lines of packs plus loose units. No database, no authorisation |
| `pl_review_counter_sale(actor, company, book, input)` | the ordinary invoice preview plus the till readout (stock on hand, change due, credit headroom) and the review hash |
| `pl_post_counter_sale(actor, company, book, input, expectedHash)` | the whole thing in one transaction with the book lock held |
| `pl_counter_sale_receipt(actor, company, book, documentId)` | the posted invoice read back as the customer's copy |

Each line posts to its product's own `sales_account_id`, resolved by `pl_counter_sale_line_account()` in the review where the product master is readable. A till never asks a cashier for a general-ledger account, and a product without one is refused by name rather than defaulted — a guessed account misclassifies revenue silently, which is the one failure a till must not have.

The cash tender is **not** what is posted. `cash_received` on the invoice is the sale total; the tender and the change
are the drawer's arithmetic, because the change never entered the business. A tender must be whole notes and coins and
a total finer than the smallest coin is refused rather than rounded (issue #88). Cash on an invoice needs the
cash-on-invoice cap set in Admin › Accounting policies first, or nothing can be paid for in cash.

**The same till serves a van.** The warehouse decides. The one behavioural difference is where the credit limit binds:
from a **fixed** location the server is present, so a credit sale over the limit is refused immediately; from a
**mobile** one it is recorded and checked at settlement, because the device may be offline. `credit_enforced_now` in
the readout says which rule applied.

**M9 consumes no migration number.** A counter sale is an invoice, a van sale is an invoice, the credit limit is the column migration 014 already added and the cost split is a read. Nothing in this milestone needs a schema change, so the numbers it might have taken stayed free for the plugin runtime (042), the sample skeletons (043) and the ownership register (044).

**The simulation.** `tools/distribution-scenario.php` records one driver's day through the shipped services — the
morning load and its gate pass, three van sales, a mid-day re-issue, a customer return that goes back into the van, the
end-of-route return, and the settlement walk. It is shared by `tools/simulate-distribution.php`, which builds a
throwaway company in the disposable `db_test` service and prints the day, and by
`tests/distribution_simulation_test.php`, which asserts every figure, so the printed walkthrough and the proof are the
same day. Neither file ships; neither is in `tools/package-files.json`.

```powershell
docker compose --profile test run --rm -e PL_DB_USER=root -e PL_DB_PASSWORD=local-test-root-only test php tools/simulate-distribution.php
docker compose --profile test run --rm test php tests/run.php --suite=counter
```
...
| `pl_assign_company_role(int $actorId, int $companyId, int $userId, int $roleId, string $reason): array` | The only way to change a membership. Writes authoritative `role_id` only; the deprecated VARCHAR `role` snapshot is retained solely for published-updater compatibility. |
...
`pl_require_company_access()` remains the membership boundary and read/write gate, resolving the scoped `role_id` and its current capabilities. `pl_can_write()` uses the same grants. Module contract 1 still receives a derived Owner/Accountant/Viewer label from `pl_role_access_label()`; it does not read the deprecated stored snapshot. Migration 056 changes that snapshot from ENUM to VARCHAR and preserves old labels for the published 1.2.1 updater; no membership service dual-writes it.
...
## Fixed assets (1.3 M14)

The `fixed-assets` bundled optional module, on `/fixed-assets`, `/fixed-assets/detail`, `/fixed-assets/depreciation` and the `/reports/asset-register` report. Its services are in `asset_functions.php`, its browser adapters in `asset_web_functions.php`, its schema in migration `046_fixed_assets`.

**It is a subsidiary ledger, not a second set of books.** Nothing in it writes a journal row: an acquisition, a depreciation run and a disposal all go through `pl_post_journal()`. Every figure the register reports is a sum of `pl_asset_events` rows and every event row is backed by one journal, so `pl_asset_register()` can compare the register with the trial balance of each control account and state the difference as a figure. A non-zero difference means something reached the account without going through the register — a manual journal straight into fixed assets is the usual one — and the report says which account and by how much rather than presenting a total that does not tie.

**Corrections are linked reversals.** `pl_asset_events` refuses UPDATE and DELETE in the database, for the same reason `pl_journal_lines` does. `pl_reverse_asset_depreciation_run()`, `pl_reverse_asset_disposal()` and `pl_reverse_asset_acquisition()` each post `pl_reverse_journal()` and write a mirror event with `is_reversal = 1` and every amount negated. A reversal event carries the **period of the row it reverses**, not the period it is dated in, which is what lets a re-run of that period compute `scheduled − posted` and post the difference rather than a duplicate.

**The convention, by owner decision of 21 September 2026: whole months.** A full month in the month an asset enters service, whatever day of it the asset arrived, and none in the month it is disposed of. It is standard, acceptable practice for a simplified asset schedule and it avoids fractional day-weighting. There is no `convention` column and that is deliberate: an earlier draft weighted accounting periods by days and offered three first-period conventions, and it was replaced rather than defaulted. A second convention later is a new column and a new decision, not a value in an enum nobody reviewed.

Each month of the life is charged to the accounting period containing **the last day of that month**, so a month is never split across two periods and a stub first fiscal period simply gets the months that end inside it.

**The arithmetic.** `pl_asset_schedule()` is a pure function — no database, no clock — so the part that has to be right can be tested with a table of numbers. Straight line allocates the *cumulative* amount, `round(depreciable x months so far / life)`, and takes each period's charge as the difference between consecutive cumulative amounts. Two things follow and both matter: four-decimal error never compounds along the life, so the charges sum to exactly cost less residual value even for a life that does not divide evenly; and opening a later accounting period never rewrites an earlier period's charge, so a posted charge cannot come to disagree with the schedule that produced it. Reducing balance takes `carrying amount x rate x months in the period / 12` and writes the remainder off in the period holding the last month of the life, which is the ordinary true-up and the only way a reducing balance reaches a residual value at all. The denominator is the **whole useful life**, never the periods that happen to exist, or a ten-year asset would be written off in the last year anybody has opened the books for.

**Methods implemented:** straight line and reducing balance. **Not implemented in 1.3:** units of production, sum-of-the-years-digits, double declining with an automatic switch, componentisation, revaluation, capitalising an existing posted bill line, opening-cutover import of an existing register, and the separate tax-depreciation basis issue #95 wants as data for a country tax plugin.

**Accounts.** The module owns four per book through `pl_asset_accounts`, keyed by a `purpose` it owns — deliberately not `pl_accounts.semantic_key`, which has exactly two writers and which migration 040 depends on having exactly two. The accumulated-depreciation pointer *reads* the chart's `core.asset.accumulated_depreciation` key and uses that account when the book has one; otherwise it creates one in the reserved 1-900 contra group (B60). A class's accumulated-depreciation account must be a contra asset, which is what makes the balance sheet present it as a deduction without a second mechanism.

**The disposal rule, stated once, because it is the part people get wrong:** "none in the month of disposal" is **not** "none in the period of disposal". An asset sold in June of an annual book is still charged for January to May. `pl_asset_settle_depreciation_to_disposal()` posts exactly those months as their own entry dated on the day of disposal, **before** the asset is removed, so the gain or loss is measured against the carrying amount on the day. A period that *ended before* the disposal is a different thing: that one the operator was supposed to run, and posting a prior year's charge into this year would be wrong, so it is refused with the period named rather than quietly fixed.

**The gain or loss on disposal goes to a single non-operating income account**, with both signs sharing it: a gain is a credit in it and a loss is a debit. Owner decision, same date. Routing it through operating expenses distorts gross margin and operating profit — selling a van is not a cost of trading — so `pl_save_asset_class()` refuses anything but an income account for it.

**The cost account's chart heading is found by `semantic_key`, never by name or number.** `pl_asset_cost_group_key()` is the single place holding the key (`core.group.asset.ppe`, the Property, Plant and Equipment heading M15 owns), and the allocator reads the group number off whatever account carries it, so this module never knows that number. Headings have their own `core.group.*` namespace because `uq_account_semantic (book_id, semantic_key)` is unique per book and a heading cannot share a key with the leaf beneath it. A chart with no such key — hand-built, or predating the headings work — falls back to the next free group below the reserved contra band, exactly as migration 039 allocated the two advances controls.

**A route is not a screen until the web server agrees.** This module's screens were on `/assets` until the pseudo-locale route sweep caught it: `www/phpledger/public/assets/` is the static mount, so the web server answered `/assets` and `index.php` never ran. `tests/asset_test.php` now refuses any manifest route whose first segment is a real directory under the document root.
