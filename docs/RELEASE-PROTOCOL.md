...
6. **Outbound contact is documented and bounded.** The update check and the installation notice are the only outbound calls; the anonymous notice is sent for every installation (owner decision, 26 September 2026) and never blocks anything when the receiver is unreachable. Their exact fields are published on the website privacy page.
...
| **Softaculous / Installatron** | Panel one-click installer calling the non-interactive install | Panel's own update notice, plus the feed | Panel extracts the new release with traffic stopped, then calls `www/phpledger/install/upgrade.php` once; the entry point runs preflight, pending migrations and a current-schema check | `panel` | Planned; [#101](https://github.com/phpledger/phpledger/issues/101). The CLI-only upgrade entry point from [#100](https://github.com/phpledger/phpledger/issues/100) exists and is packaged; panel integration and a published-channel upgrade proof remain open. The panel operator must arrange a matched backup before extraction. |
...
11. **Upgrade proof.** On a disposable copy of the previous release, run the upgrade path for every open channel: in-app update for ZIP, tag pull for the container, and `www/phpledger/install/upgrade.php` for an opened panel package. Record each result.
