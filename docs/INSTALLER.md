...
12. **Six stages, laid out as the Workbench** (owner decision, 20 September 2026, direction B in `docs/design/installer-2026-09`). Start, Database, Build, Checks, Account and Ready, shown as a compartment tray, each stage sized for a laptop screen without scrolling.
...
13. **Server checks, a progress bar and few requests for the schema** (owner feedback, 20 September 2026). Setup used to apply one schema step per browser request and show a submit button while it worked.
...
14. **A database on this same server** (owner decision, 20 September 2026, [issue #84](https://github.com/phpledger/phpledger/issues/84)). `localhost`, `127.0.0.1` and `::1` mean the owner's own machine or hosting account, the same test that already decides the one-time remote code.
