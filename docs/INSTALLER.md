...
10. **A database on this same server** (owner decision, 20 September 2026, [issue #84](https://github.com/phpledger/phpledger/issues/84)). `localhost`, `127.0.0.1` and `::1` mean the owner's own machine or hosting account, the same test that already decides the one-time remote code.
