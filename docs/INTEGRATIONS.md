...
The existing source and transaction-list read DTOs expose nullable `party_id` in addition to `counterparty`. The ID identifies a party in the same company; the name remains a document snapshot, including in correction revisions. Legacy sources remain null and are never linked by a name guess. Party identity alone creates no AR/AP recognition or allocation. No public financial-write endpoint is introduced. The additive `057_document_parties` migration retains old immutable source rows and extends the effective-document view for current correction snapshots. This extension is locally implemented and awaits the combined validation recorded in the implementation receipt.
