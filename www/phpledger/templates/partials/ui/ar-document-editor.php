...
$fields=['party_id'=>['ar-party','Customer / supplier'],'date'=>['ar-date','Document date'],'due_date'=>['ar-due','Due date'],'currency'=>['ar-currency','Currency'],'reference'=>['ar-reference','Reference']];
foreach (array_values($v['lines']??[[]]) ?: [[]] as $index=>$unused) {
    foreach (['description'=>'Description','quantity'=>'Quantity','unit_price'=>'Unit price'] as $field=>$text) { $fields['lines.'.$index.'.'.$field]=['commercial-'.$index.'-'.$field,$text.', line '.($index+1)]; }
}
?>
<div class="py-5"><section class="rounded-panel border border-border bg-surface">
<?php pl_ui_document_header(($correct?'Correct ':($document?'':'New ')).($document?$document['number']:$label),$correct?'posted':'draft',static function () use ($correct,$path,$postingPreview,$filters): void { ?>
<a class="btn btn-ghost" href="<?= pl_e(pl_workflow_url($path,$filters)) ?>">Cancel</a>
<?php if ($correct): ?><button class="btn btn-secondary" form="ar-document-editor" name="editor_action" value="preview">Update correction preview</button><?php if ($postingPreview): ?><button class="btn btn-primary" form="ar-document-editor" name="editor_action" value="post_reviewed_document" data-review-confirm>Reverse and post correction</button><?php endif; ?>
...
pl_starter_field('Reference','reference',$v['reference']??'','text',false,error:$errors['reference']??'',id:'ar-reference'); pl_starter_field('Terms','terms',$v['terms']??'','text',false); pl_starter_field('Notes','notes',$v['notes']??'','text',false); ?>
</section>
<?php if ($tradingOn): ?>
...
<?php if (isset($postingPreview['reversal'])): ?><section data-review-preview><h2 class="section-title mb-2">Reversal on <?= pl_e(pl_date_label($postingPreview['reversal']['date'])) ?></h2>
<?php pl_ui_table(['Account','Debit ('.$postingPreview['currency'].')','Credit ('.$postingPreview['currency'].')'],static function () use ($postingPreview,$names):void { foreach ($postingPreview['reversal']['lines'] as $line): ?><tr><td><?= pl_e($names[$line['account_id']]??('Account '.$line['account_id'])) ?></td><td class="amount"><?= pl_e(pl_money($line['debit'])) ?></td><td class="amount"><?= pl_e(pl_money($line['credit'])) ?></td></tr><?php endforeach; },'Correction reversing entry'); ?>
<p class="text-xs text-ink-muted mt-2">The original journal stays unchanged. This reversal and the replacement below post together, or neither posts.</p></section><?php endif; ?>
<?php if (($postingPreview['stock_reversal']??[])!==[]): ?><section data-review-preview><h2 class="section-title mb-2">Stock restored before replacement</h2>
<?php foreach ($postingPreview['stock_reversal'] as $effect): ?><p class="text-xs text-ink-muted"><?= pl_e($effect['product_name'].' · Quantity change '.$effect['quantity_delta'].' · Carrying value change '.$company['currency'].' '.$effect['value_delta']) ?></p><?php endforeach; ?></section><?php endif; ?>
...
<?php if (($postingPreview['stock']??[])!==[]): ?><section data-review-preview><details open><summary class="section-title mb-2">Stock and cost effects</summary>
<?php foreach ($postingPreview['stock'] as $stock): ?><p class="text-xs text-ink-muted my-2"><?= pl_e($stock['product_name']) ?> · Quantity change <?= pl_e($stock['quantity_delta']) ?> · Carrying value change <?= pl_e($company['currency'].' '.$stock['value_delta']) ?></p>
<?php pl_ui_table(['Account','Debit','Credit'],static function () use ($stock,$names): void { foreach ($stock['movement']['journal']['lines']??[] as $line): ?><tr><td><?= pl_e($names[$line['account_id']]??('Account '.$line['account_id'])) ?></td><td class="amount"><?= pl_e(pl_money($line['debit'])) ?></td><td class="amount"><?= pl_e(pl_money($line['credit'])) ?></td></tr><?php endforeach; },'Linked stock journal effect'); ?>
