...
<label class="field"><?= pl_e(pl_t('Person\'s # in that register')) ?><input class="input" name="related_id" inputmode="numeric" required value="<?= pl_e(pl_web_text($input, 'related_id')) ?>"><span class="field-hint"><?= pl_e(pl_t('The # shown against them in the directors/officers or members table above, or on the Employees screen if key management personnel is an employee rather than a director.')) ?></span></label>
