...
<label class="field">From<input class="input" type="date" name="from" value="<?= pl_e($from) ?>" required></label><label class="field">To<input class="input" type="date" name="to" value="<?= pl_e($to) ?>" required></label>
<div class="field"><label for="pl-depth">Depth shown</label><select id="pl-depth" class="select" name="depth"><?php foreach (pl_report_depth_options() as $value=>$label): ?><option value="<?= (int)$value ?>" <?= $depth===$value?'selected':'' ?>><?= pl_e($label) ?></option><?php endforeach; ?></select></div>
<button class="btn btn-primary">Update</button><a class="btn btn-ghost" href="<?= pl_e(pl_url('/reports')) ?>">All reports</a>
</form>
...
<section class="flex flex-col gap-2" aria-labelledby="pl-<?= pl_e($key) ?>"><h2 class="section-title" id="pl-<?= pl_e($key) ?>"><?= pl_e($label) ?></h2>
<?php pl_ui_report_tree($trees[$key], $columns, ['caption' => $label . ' in ' . $company['currency'],
    ...
    'empty' => $key==='cost_of_sales' ? 'No accounts classified as cost of sales. Existing expenses stay in Expenses until explicitly classified.' : 'No ' . strtolower($label) . ' accounts carry a balance in this period.']); ?>
<table class="stmt-table"><caption class="sr-only">Total <?= pl_e(strtolower($label)) ?></caption><tbody><tr class="stmt-row stmt-subtotal"><th scope="row">Total <?= pl_e(strtolower($label)) ?></th><td class="num"><?= pl_e(pl_money($report['total_'.$key])) ?></td></tr>
...
<?php endforeach; ?>
<table class="stmt-table"><caption class="sr-only">Net result</caption><tbody><tr class="stmt-row stmt-total"><th scope="row">Net profit / loss</th><td class="num"><?= pl_e(pl_money($report['net_profit'])) ?></td></tr></tbody></table>
</div>
<p class="text-xs text-ink-muted">Sales returns and discounts allowed are shown as deductions inside Income, and purchase returns and discounts received as deductions inside their expense section; they are never moved to the opposite side. Includes posted entries and dated reversals. Account links retain this date range. Cost-of-sales classification changes presentation across all periods; it does not change posted entries or net profit.</p>
</div>
