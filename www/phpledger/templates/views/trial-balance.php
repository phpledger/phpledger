...
<table class="table"><caption class="sr-only">Trial balance totals</caption><tfoot><tr class="table-totals"><th scope="row">Total</th><td class="num"><?= pl_e(pl_money($report['total_debit'])) ?></td><td class="num"><?= pl_e(pl_money($report['total_credit'])) ?></td></tr></tfoot></table>
<?php if ($report['balanced']): ?><p class="balanced-label"><?= pl_icon('check') ?> Debit and credit totals agree</p><?php endif; ?>
<p class="text-xs text-ink-muted">Each class and group adds up the accounts inside it; expand a class to see its groups and accounts, or print the page to keep exactly what you have expanded. Contra accounts such as accumulated depreciation and drawings are shown as deductions inside their own section, so the totals still agree. Select an account to see its posted activity. Equal totals confirm that posted debits and credits balance. They do not by themselves establish that every transaction has been recorded or categorized correctly.</p>
</section>
