...
<section class="flex flex-col gap-4 py-5" aria-labelledby="help-title">
    ...
    <div class="rounded-panel border border-border bg-surface p-4">
        ...
        <p><?= pl_e(pl_t('A balanced journal does not prove that cash was available. Choose Warning only or Strict in Admin > Accounting policies. Warning only, the default, permits recording a shortfall. Strict blocks payments that create or worsen a cash shortfall, including on later dates affected by a backdated entry. Drafts do not reserve cash. Record actual funding or correct the payment account before posting; in Strict mode, bank payments are checked against zero or an explicitly configured agreed overdraft limit. A bank book balance and this limit check do not confirm bank clearance or available funds.')) ?></p>
