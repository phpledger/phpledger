...
<div class="table-wrap" tabindex="0" role="region" aria-label="Batch rows; scroll horizontally on smaller screens"><table class="table"><thead><tr><th><?= $receipt?'Customer':'Vendor' ?></th><th class="text-end">Outstanding before</th><th><?= $receipt?'Received':'Paid' ?> <?= pl_e($company['currency']) ?></th><th class="text-end">Applied</th><th class="text-end">Held on account<?php pl_ui_help('unapplied-credit', 'sheet'); ?></th></tr></thead><tbody>
<?php
...
<div class="text-xs text-ink-muted mb-3">Enter the amount to allocate to each document; leave the others blank. Up to <?= pl_settlement_allocation_cap() ?> allocations; split a larger recovery into several receipts. Allocations may not exceed the payment. Whatever is left over is held as unapplied credit on the advances control above — it is never dropped and never guessed.<?php pl_ui_help('unapplied-credit'); ?></div>
