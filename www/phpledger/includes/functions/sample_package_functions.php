...
/**
 * The bundled snapshot of the phpledger.com sample directory (approved frame o1-start.html, 1.4.6):
 * the eleven companies with their story, logo and what their structure brings, so the Start stage
 * and the Packages Directory tab show them on every copy before anything is fetched or installed.
 * Generated by tools/build-sample-packages.py from the authoring catalogue; never edited by hand.
 *
 * @return array<string,array{id:string,slug:string,version:string,name:string,business:string,story:string,description:string,logo:string,accounts:int,parties:int,modules:list<string>}>
 */
function pl_sample_catalogue(): array
{
    static $catalogue=null;
    if($catalogue!==null){return $catalogue;}
    $path=PL_ROOT.'/resources/sample-catalogue.json';
    if(!is_file($path)||is_link($path)){throw new RuntimeException('The bundled sample catalogue is missing.');}
    $data=json_decode((string)file_get_contents($path),true,16,JSON_THROW_ON_ERROR);
    if(!is_array($data)||($data['schema']??null)!==1||!is_array($data['samples']??null)||!array_is_list($data['samples'])||count($data['samples'])>200){throw new RuntimeException('The bundled sample catalogue is invalid.');}
    $result=[];
    foreach($data['samples'] as $entry){
        $id=$entry['id']??null;
        if(!is_string($id)||!preg_match('/^[a-z][a-z0-9-]{1,60}$/D',$id)||isset($result[$id])||($entry['slug']??null)!=='sample-'.$id
            ||!is_string($entry['version']??null)||!preg_match('/^[0-9]+\.[0-9]+\.[0-9]+$/D',$entry['version'])
            ||!is_string($entry['name']??null)||trim($entry['name'])===''||!is_string($entry['business']??null)||!is_string($entry['story']??null)
            ||!is_string($entry['description']??null)||!is_int($entry['accounts']??null)||!is_int($entry['parties']??null)||!is_array($entry['modules']??null)
            ||!is_string($entry['logo']??null)||($entry['logo']!==''&&$entry['logo']!=='/assets/sample-companies/'.$id.'.svg')){
            throw new RuntimeException('The bundled sample catalogue is invalid.');
        }
        $result[$id]=['id'=>$id,'slug'=>$entry['slug'],'version'=>$entry['version'],'name'=>$entry['name'],'business'=>$entry['business'],'story'=>$entry['story'],
            'description'=>$entry['description'],'logo'=>$entry['logo'],'accounts'=>$entry['accounts'],'parties'=>$entry['parties'],'modules'=>array_values(array_map('strval',$entry['modules']))];
    }
    return $catalogue=$result;
}
/**
 * What this copy can offer from phpledger.com, keyed by slug: the bundled snapshot, with the
 * fetched directory's newer versions and any sample published after this release on top.
 *
 * @return array<string,array<string,mixed>>
 */
function pl_sample_directory_offer(): array
{
    $offer=[];
    foreach(pl_sample_catalogue() as $entry){
        $offer[$entry['slug']]=['type'=>'sample','slug'=>$entry['slug'],'version'=>$entry['version'],'name'=>$entry['name'],'description'=>$entry['description'],
            'author'=>'PHP Ledger','licence'=>'CC0-1.0','homepage'=>'https://phpledger.com/directory/'.$entry['slug'].'/',
            'metadata_url'=>'https://phpledger.com/directory/'.$entry['slug'].'/'.$entry['version'].'/sample-envelope.json',
            'bundled'=>true,'business'=>$entry['business'],'story'=>$entry['story'],'logo'=>$entry['logo'],'accounts'=>$entry['accounts'],'parties'=>$entry['parties']];
    }
    foreach(pl_sample_directory_cached()['packages'] as $entry){
        $slug=(string)$entry['slug'];
        if(isset($offer[$slug])){
            if(version_compare((string)$entry['version'],$offer[$slug]['version'],'<')){continue;}
            $offer[$slug]=array_replace($offer[$slug],['version'=>(string)$entry['version'],'name'=>(string)$entry['name'],'description'=>(string)$entry['description'],'metadata_url'=>(string)$entry['metadata_url'],'bundled'=>false]);
        }else{
            $offer[$slug]=$entry+['bundled'=>false,'business'=>'','story'=>(string)$entry['description'],'logo'=>'','accounts'=>0,'parties'=>0];
        }
    }
    return $offer;
}
...
function pl_sample_directory_install(int $actor,string $slug,string $key): array
{
    ...
    if($entry===null&&isset(pl_sample_directory_offer()[$slug])){
        // One click on a bundled card: fetch the directory now and continue with what it lists.
        // Install already contacts phpledger.com, so the refresh adds no new destination.
        foreach(pl_sample_directory_refresh($actor)['packages'] as $candidate){if($candidate['slug']===$slug){$entry=$candidate;break;}}
    }
