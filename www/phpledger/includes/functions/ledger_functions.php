...
function pl_cash_payment_preview(int $actorId, int $companyId, int $bookId, int $accountId, string $date, string $amount): array
{
    ...
    if ($account['money_kind'] === null) { $warning = 'Choose physical cash or bank in the chart of accounts before posting money out.'; }
    return ['account_id' => $accountId, 'name' => $account['name'], 'role' => $account['role'], 'money_kind' => $account['money_kind'], 'currency' => $book['currency'], 'date' => $date,
        'balance' => $balance, 'payment' => $amount, 'projected_balance' => $projected, 'insufficient_cash' => $insufficient, 'blocked_payment' => $insufficient || ($account['money_kind'] === null && bccomp($amount, '0', 4) > 0), 'warning' => $warning,
