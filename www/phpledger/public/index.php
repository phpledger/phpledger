...
try {
    ...
    if ($path === '/transactions/save') {
        ...
        try {
            ...
            $action = pl_web_text($_POST, 'editor_action');
            if (in_array($action, ['search_party','open_party','cancel_party','create_party'], true)) {
                ...
                if ($action === 'create_party') {
                    $created = pl_save_party($actorId,$companyId,$bookId,[
                        'legal_name'=>pl_web_text($_POST,'new_party_name'), 'entity_type'=>pl_web_text($_POST,'new_party_type','business'),
                        'country_code'=>strtoupper(pl_web_text($_POST,'new_party_country')), 'currency'=>$company['currency'],
                        'is_customer'=>isset($_POST['new_party_customer']), 'is_vendor'=>isset($_POST['new_party_vendor']),
                        'request_key'=>pl_web_text($_POST,'party_creation_key'), 'reason'=>'Created from a receipt or expense draft.',
                    ]);
                    $_POST['party_id'] = (string)$created['id'];
                    $_POST['counterparty'] = pl_web_text($_POST,'new_party_name');
                    $_POST['party_search'] = '';
                    $_POST['party_creation_key'] = bin2hex(random_bytes(24));
                    $_POST['editor_action'] = 'party_created';
                }
                if ($action === 'open_party') { $_POST['new_party_name'] = pl_web_text($_POST,'party_search'); }
                pl_form_failure($return,$_POST,'',200);
            }
            if (($input['party_id'] ?? null) === null) {
                $legacy = $documentId ? pl_get_document($actorId,$companyId,$bookId,$documentId) : null;
    ...
    if ($path === '/transactions/new' || $path === '/transactions/edit') {
        ...
        if (!$form['input'] && !$document && !isset($_GET['return_filters']) && in_array(pl_web_text($_GET,'kind'),['receipt','expense'],true)) { $returnFilters['kind'] = $input['kind']; }
        ...
        pl_render('editor', ['title' => $document ? 'Edit draft' : ($input['kind'] === 'receipt' ? 'New receipt' : 'New expense'), 'user' => $user, 'company' => $company, 'document' => $document, 'input' => $input, 'form' => $form, 'preview'=>$preview, 'returnFilters'=>$returnFilters,'parties'=>$parties,'cashPreview'=>$cashPreview]);
    }
